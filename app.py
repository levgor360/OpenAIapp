...
# App title
st.set_page_config(page_title="Future Forecaster")
...
with st.sidebar:
    ...
    os.environ['REPLICATE_API_TOKEN'] = openai_api_key
    ...
    # max_length = st.sidebar.slider('max_length', min_value=32, max_value=10000, value=120, step=8)
...
Using the instructions described under the heading "Instructions" and shaping the narrative in a way that is portrayed in the section "Examples", develop a 'future scenario' about 300-words long. The narrative should reflect elements described in the lists under the sub-heading "Opportunities" and sub-heading "Dangers" as described in the section under the heading "Instructions". The narrative should weave the elements described in the lists under the sub-heading "Opportunities" and sub-heading "Dangers" in a subtle manner. Do not point out when an element from the lists "Opportunities" and "Dangers" has impacted the narrative. Do not explicitly state to the reader when an exemplary case of an Opportunity or Danger has been described. 

If there are no elements listed under the sub-heading "Opportunities" and sub-heading "Dangers", respond with this text verbatim: "Please insert a description of opportunities and dangers in the slots provided to the left."
